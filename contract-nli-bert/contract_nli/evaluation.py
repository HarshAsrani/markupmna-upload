...
def evaluate_class(y_true, y_prob) -> Dict[str, float]:
    assert y_prob.ndim == 2 and y_prob.shape[1] == 11
    assert y_true.ndim == 1
    assert len(y_true) == len(y_prob)
    ...
    for label in (NLILabel.ZERO_VAL, NLILabel.ONE_VAL, NLILabel.TWO_VAL,
            NLILabel.THREE_VAL, NLILabel.FOUR_VAL, NLILabel.FIVE_VAL,
            NLILabel.SIX_VAL, NLILabel.SEVEN_VAL, NLILabel.EIGHT_VAL,
            NLILabel.NINE_VAL):
        ln = label.name.lower()
...
def remove_not_mentioned(y_pred):
    assert y_pred.shape[1] == 11
    y_bin = y_pred[:, [NLILabel.ZERO_VAL.value, NLILabel.ONE_VAL.value, NLILabel.TWO_VAL.value,
            NLILabel.THREE_VAL.value, NLILabel.FOUR_VAL.value, NLILabel.FIVE_VAL.value,
            NLILabel.SIX_VAL.value, NLILabel.SEVEN_VAL.value, NLILabel.EIGHT_VAL.value,
            NLILabel.NINE_VAL.value]]
    y_bin = np.where(np.tile(np.sum(y_bin, axis=1, keepdims=True), [1, 10]) == 0,
                     0.5,
                     y_bin / np.sum(y_bin, axis=1, keepdims=True))
    y_pred = np.zeros((len(y_pred), 11), dtype=y_pred.dtype)
    y_pred[:, [NLILabel.ZERO_VAL.value, NLILabel.ONE_VAL.value, NLILabel.TWO_VAL.value,
            NLILabel.THREE_VAL.value, NLILabel.FOUR_VAL.value, NLILabel.FIVE_VAL.value,
            NLILabel.SIX_VAL.value, NLILabel.SEVEN_VAL.value, NLILabel.EIGHT_VAL.value,
            NLILabel.NINE_VAL.value]] = y_bin
    return y_pred
...
def evaluate_all(
    ...
    if task in ['identification_classification', 'classification']:
        binary_label_ids = [
            ...
            if NLILabel.ONE_VAL.value in class_labels[l] or
               NLILabel.TWO_VAL.value in class_labels[l] or
               NLILabel.ZERO_VAL.value in class_labels[l] or
               NLILabel.THREE_VAL.value in class_labels[l] or
               NLILabel.FOUR_VAL.value in class_labels[l] or
               NLILabel.FIVE_VAL.value in class_labels[l] or
               NLILabel.SIX_VAL.value in class_labels[l] or
               NLILabel.SEVEN_VAL.value in class_labels[l] or
               NLILabel.EIGHT_VAL.value in class_labels[l] or
               NLILabel.NINE_VAL.value in class_labels[l]]
