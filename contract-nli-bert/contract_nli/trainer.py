...
class Trainer(object):
    ...
    def effective_batch_size(self) -> int:
        n_gpus = torch.cuda.device_count() if self.local_rank != -1 else 1
    ...
    def train(self):
        ...
        while (self.global_step + 1) <= self.max_steps:
            ...
            for batch in self.train_dataloader:
                ...
                losses_span.append(loss_span.item())
        ...
        for item in losses:
	        file.write(str(item)+"\n")
        file.close()
        file = open('losses_cls.txt','w')
        for item in losses_cls:
            file.write(str(item)+"\n")
        file.close()
        file = open('losses_span.txt','w')
        for item in losses_span:
            file.write(str(item)+"\n")
        file.close()
