...
class BertForIdentificationClassification(BertPreTrainedModel):
    ...
    def __init__(self, config):
        ...
        self.class_outputs = nn.Linear(config.hidden_size, 11)
    ...
    def forward(
        ...
        if class_labels is not None:
            ...
            num_classes = logits_cls.size(1)  # Assuming logits_cls is the output of your model
            if torch.all(class_labels == -100):
                loss_cls = torch.tensor(0.).to(logits_cls.device)
            else:
